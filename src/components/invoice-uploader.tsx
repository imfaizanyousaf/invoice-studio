...
interface InvoiceUploaderProps {
  ...
  uploadLimit?: number | null; // New prop: if null, no limit is applied
}
...
export function InvoiceUploader({
  ...
  uploadLimit = null, // New prop with default value
  ...
  const onDrop = useCallback(
    ...
    [onFilesAccepted, isProcessing, uploadLimit, uploadedFiles.length]
  );
  ...
  const isLimitReached =
    uploadLimit !== null && uploadedFiles.length >= uploadLimit;
          ...
          <CardTitle className="flex items-center justify-between text-2xl font-semibold text-foreground">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-auto min-w-[10rem]">
                <DropdownMenuItem
                  onSelect={() => onInvoiceTypeChange("purchase")}
                >
                  Purchase Invoice
                </DropdownMenuItem>
        ...
        <CardContent className="p-6 md:p-8">
          <div
            ...
            className={`p-8 md:p-12 border-2 border-dashed rounded-lg cursor-pointer transition-all duration-150 ease-in-out
              ${
                isDragActive && !isProcessing && !isLimitReached
                  ? "border-primary bg-primary/10 scale-105 shadow-inner"
                  : "border-border hover:border-primary/70"
              }
              ${
                isProcessing || isLimitReached
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center text-center min-h-[120px]">
              {isLimitReached ? (
                <>
                  <XCircle className="h-12 w-12 mb-4 text-muted-foreground" />
                  <p className="text-xl font-semibold text-muted-foreground">
                    Upload limit reached
                  </p>
                  <p className="text-sm text-muted-foreground mt-1">
                    You've reached the maximum of {uploadLimit} invoices. Sign
                    up to upload more.
                  </p>
                </>
                ...
                <>
                  <UploadCloud
                    className="h-12 w-12 mb-4 text-primary animate-bounce"
                ...
                <>
                  <UploadCloud
                    className="h-12 w-12 mb-4 text-muted-foreground group-hover:text-primary transition-colors"
                    data-ai-hint="cloud upload"
                  />
              ...
              <ScrollArea className="h-40 rounded-md border">
                <div className="p-2 space-y-1">
                  {uploadedFiles.map((invoice) => (
                    <div
                      key={invoice.id}
                      className="flex items-center justify-between p-2.5 rounded-md hover:bg-muted/20 transition-colors duration-150 ease-in-out"
                    >
          ...
          {rejectedFiles.length > 0 && !isProcessing && (
            <div className="mt-6 p-4 bg-destructive/10 border border-destructive/30 rounded-lg">
              ...
              <Button
                ...
                className="mt-3 text-destructive hover:bg-destructive/20 h-auto py-1.5 px-2.5"
              >
                Clear Rejected
              </Button>
            </div>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-left">
              You've reached the maximum upload limit of {uploadLimit} invoices.
              Sign in to avail 10 free credits.
            </DialogDescription>
          </DialogHeader>
